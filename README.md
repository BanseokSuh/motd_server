...
- 로그인/회원가입
  - Spring Security를 이용한 JWT 토큰 기반 인증
...
- 댓글/좋아요 기능
- 스레드 위임을 통한 비동기 이메일 발송
  - 회원가입 완료 후 welcome 이메일
  - Thymeleaf를 이용한 이메일 템플릿 작성
- Logback을 이용한 로그 관리
  - Profile에 따른 로그 레벨 설정
- SSE/Kafka를 이용한 실시간 알림
  - 댓글 작성, 좋아요 시 작성자에게 실시간 메시지 발송

[//]: # (TODO: 기능 추가 예정)
[//]: # (- H2, Embedded Redis를 활용한 테스트 환경 격리)
[//]: # (- 승인되지 않은 유저에 대한 접근 권한 처리)
[//]: # (- 게시글 신고 기능)
[//]: # (- 게시글 북마크 기능)
